...
export function Projects({ lang }: ProjectsProps) {
    const projects = PROJECTS.map((project) => {
        const links = resolvePoly(project.links, lang);
        const resolvedLinks = links?.map((link) => ({
            ...link,
            title: link.labelKey
                ? t(lang, link.labelKey as any)
                : resolvePoly(link.title, lang),
            url: resolvePoly(link.url, lang),
        }));

        return {
            ...project,
            title: resolvePoly(project.title, lang),
            description: resolvePoly(project.description, lang),
            detailedDescription: project.detailedDescription
                ? resolvePoly(project.detailedDescription, lang)
                : undefined,
            technologies: resolvePoly(project.technologies, lang),
            links: resolvedLinks,
        };
    });
    ...
    return (
        <section id="projects" className="flex flex-col pt-6 mt-18 gap-8 relative">
            <h2 className="text-2xl font-bold flex items-center gap-2">
                <span className="text-primary-400">/</span> {t(lang, "nav.projects")}
            </h2>
            ...
            {projects.length > INITIAL_COUNT && (
                <div className="flex justify-center">
                    <Button variant="outline" onClick={() => setShowAll(!showAll)}>
                        {showAll ? t(lang, "projects.showLess") : t(lang, "projects.viewMore")}
                    </Button>
                </div>
            )}
