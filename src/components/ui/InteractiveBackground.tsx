...
export function InteractiveBackground() {
    ...
    useEffect(() => {
        ...
        // PRE-CALCULATION
        // ---------------------------------------------
        // We use Math.PI * 2 constantly, let's cache if needed,
        // but JS engines optimize this well.
        const PI2 = Math.PI * 2;
        ...
        const render = () => {
            ...
            const speed = Math.abs(HOVER_MAX_RADIUS - BASE_MAX_RADIUS) / transitionDuration;
            ...
            // We can optimize by calculating dot properties ONCE and storing them?
            // Or just loop twice? Looping twice is cheaper than allocating 2000 objects per frame.
            ...
            const calculateRadius = (x: number, y: number) => {
                ...
                if (distMouse < MOUSE_INFLUENCE_RADIUS) {
                    const growth = 1 - distMouse / MOUSE_INFLUENCE_RADIUS;
                    // Use the interpolated "Super Size" for mouse interactions
                    r += (currentMaxRadius - DOT_BASE_RADIUS) * growth;
                }

                // 2. Attractor Interaction (ALWAYS stays at Base Size)
                for (const rect of activeAttractors) {
                    ...
                    if (distAttractor < ATTRACTOR_INFLUENCE_RADIUS) {
                        ...
                        // Use BASE_MAX_RADIUS directly, ignoring the hover boost
            ...
            ctx.fillStyle = "rgba(183, 134, 0, 0.6)";
